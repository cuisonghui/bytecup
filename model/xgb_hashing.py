...
#    ulabel = pd.DataFrame(ulabel, columns=('ulabel_{}'.format(i) for i in range(ULABEL_NUM)))
    ...
    hasher = FeatureHasher(n_features=30)
    ulabel = []
    for label in user.ulabel:
        if label == '/':
            ulabel.append({})
        else:
            label = label.split('/')
            ulabel.append({x: 1 for x in label})
    ulabel = hasher.transform(ulabel).toarray()
    ulabel = pd.DataFrame(ulabel, columns=('ulabel_{}'.format(i) for i in range(ulabel.shape[1])))
    ...
    pred.to_csv('output/xgb_hashing.csv', index=False)
